...
class Program():
    ...
    def __init__(self, master=None, number=0, commands=[]):
    ...
    # Set the compiled program text
    def setCompiledProgram(self, p):
        try:
            assert(isinstance(p, str))
        except AssertionError:
            print("Error: Argument of setCompiledProgram must be a string.")
            return

        self.compiledProgram = p
    ...
    def compile(self):
        print("\t\tCompiling program %i" % self.number)
            ...
            # If there are no problems with syntax, attempt to add the command to the compiled program
                ...
                if c.__contains__('(') and not c.startswith('('):
                    ...
                    if args:
                        ...
                        for arg in args:
                            try:
                                command.getVariables()[count].setCurrentValue(arg)
                            except IndexError:
